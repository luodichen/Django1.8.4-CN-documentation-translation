...
----------
#### 在 Jython 上运行 Django ####
如果要在 Jython（一个 Java 平台上的 Python 实现） 上运行 Django，需要执行几个额外的步骤，详情请参见 [在 Jython 上运行 Django](?)
...
- 如果要使用 PostgreSQL，需要安装 [psycopg2](http://initd.org/psycopg/) 包。详情请见[PostgreSQL notes](?)。
- 如果要使用 MySQL，需要安装一个 [DB API driver](?)，例如 `mysqlclient`。详情请见 [notes for the MySQL backend](?)。
...
如果要使用 Django 提供的 `manage.py migrate` 命令来为模块自动创建数据库表（在第一次安装 Django 并创建一个项目之后），需要确保 Django 拥有创建和修改数据库表的权限。如果是手动创建数据库表，可以只授予 Django `SELECT`、`INSERT`、`UPDATE` 和 `DELETE` 操作的权限。在创建好数据库用户并为它授权之后，还需要在项目的配置文件中配置数据库的详细信息，详情参见 [DATABASES](?)。
...
如果你希望能够随时更新 Django 代码到最后一次 bug 修复及改进，参照以下指示：
...
这样， Django 的代码就可以被引用，同时， `django-admin` 工具命令也可用。换句话说，一切准备就绪。<br />
...
（请注意应该在系统的命令提示符中运行这个命令，而不是在 Python 的交互式命令提示符下运行。）

某些基于 Debian 的 Linux 发行版会为用户自行安装的包创建单独的 `site-package` 目录，例如从一个下载的压缩包中安装 Django 时就会如此。上面列出的命令只会显示系统的 `site-package` 目录，要查看用户的 `site-package` 目录，可以将系统 `site-package` 目录中的 `/usr/lib/` 替换为 `/usr/local/lib/`。

接下来需要使 `django-admin.py` 工具在 shell 的 PATH 环境中可用。

在类 Unix 系统中，在系统目录例如 `/usr/local/bin` 中创建一个符号链接指向文件 `django-trunk/django/bin/django-admin`，下面是一个示例：
